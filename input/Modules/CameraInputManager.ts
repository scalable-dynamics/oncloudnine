export class CameraInputManager implements IMediaInputManager {
    isRecording: boolean;
    ...
    async startRecording() {
        if (!this.isRecording && navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            try {
                ...
                console.error('Error starting camera input stream:', err);
            }
        }
    }
    ...
    async capture() {
        ...
        return { type: 'camera', dataUrl } as IMediaInput;
    }
}
